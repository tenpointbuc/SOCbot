...
Env:
  ...
  NOCSOC_N8N_URL       n8n base url (default http://<host_ip>:<n8n port>)
...
def n8n_url():
    ...
    if base:
        return base.rstrip("/")
    host = cfg.dig(cfg.load_site(), "network.host_ip", "127.0.0.1")
    port = 5678
...
def api(method, path, body=None):
    key = os.environ.get("NOCSOC_N8N_API_KEY")
    if not key:
        raise ImportError_("NOCSOC_N8N_API_KEY unset (secret) — cannot call n8n API")
...
def provision_credentials():
    ...
    for spec in load_cred_specs():
        ...
        for field, secret_key in (spec.get("data") or {}).items():
            env_key = "NOCSOC_SECRET_%s" % secret_key
            val = os.environ.get(env_key)
            if val is None:
                raise ImportError_("credential '%s' needs secret %s (%s unset)"
                                   % (name, secret_key, env_key))
            fields[field] = val
